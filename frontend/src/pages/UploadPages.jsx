...
import { ArrowLeft, Clapperboard, PlusSquare, CircleUserRound, Upload, X, Image as ImageIcon, Video, PartyPopper } from 'lucide-react';
...
const UploadPages = () => {
  ...
  const handleFileChange = (event) => {
    ...
    // ✅ FIX: Validate file type based on selected option
    if (selectedOption === 'post' && !selectedFile.type.includes('image')) {
      alert('Posts only accept images');
      return;
    }
  ...
  const handleUpload = () => {
    ...
    console.log('Uploading:', { type: selectedOption, file: backendmedia, caption, mediaType });
  };

  // ✅ FIX: Determine file accept attribute based on selected option
  const getAcceptedFileTypes = () => {
    if (selectedOption === 'post') return 'image/*';
        ...
        {!selectedOption ? (
          ...
          <div className="w-full max-w-4xl">
            <div className="text-center mb-8 lg:mb-12">
              ...
              <p className="text-gray-600 text-sm sm:text-base">Choose an option to get started</p>
            </div>
              ...
              >
                <div className="flex flex-col items-center text-center">
                  <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-purple-100 group-hover:bg-purple-600 flex items-center justify-center transition-all duration-300 group-hover:scale-110 mb-4">
                    <PlusSquare size={40} className="text-purple-600 group-hover:text-white transition-colors" />
                  </div>
              ...
              >
                <div className="flex flex-col items-center text-center">
                  <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-pink-100 group-hover:bg-pink-600 flex items-center justify-center transition-all duration-300 group-hover:scale-110 mb-4">
                    <Clapperboard size={40} className="text-pink-600 group-hover:text-white transition-colors" />
                  </div>
              ...
              >
                <div className="flex flex-col items-center text-center">
                  <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-blue-100 group-hover:bg-blue-600 flex items-center justify-center transition-all duration-300 group-hover:scale-110 mb-4">
                    <CircleUserRound size={40} className="text-blue-600 group-hover:text-white transition-colors" />
                  </div>
          ...
          <div className="w-full max-w-2xl">
            <div className="bg-white rounded-2xl shadow-lg p-8 lg:p-12 border border-gray-200">
              <div className="flex flex-col items-center justify-center text-center">
                <div className={`w-24 h-24 lg:w-32 lg:h-32 rounded-full flex items-center justify-center mb-6 ${
                  selectedOption === 'post' ? 'bg-gradient-to-br from-purple-100 to-pink-100' :
                  selectedOption === 'reel' ? 'bg-gradient-to-br from-pink-100 to-red-100' :
                  ...
                  {selectedOption === 'reel' ? (
                    ...
                    <div className="relative">
                      ...
                      <Video size={24} className="text-blue-600 absolute -bottom-2 -right-2" />
                    </div>
                  ) : (
                ...
                <h2 className="text-2xl lg:text-3xl font-bold text-gray-800 mb-3">
                  {selectedOption === 'post' && 'Select an Image'}
                ...
                <p className="text-gray-600 mb-2 text-sm lg:text-base">
                  {selectedOption === 'post' && 'Choose an image to share with your followers'}
                  {selectedOption === 'reel' && 'Select a video to create your reel'}
                  {selectedOption === 'story' && 'Pick an image or video for your 24-hour story'}
                </p>
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs mb-6 ${
                  selectedOption === 'post' ? 'bg-purple-100 text-purple-700' :
                  selectedOption === 'reel' ? 'bg-pink-100 text-pink-700' :
                ...
                <button
                  ...
                  className={`flex items-center gap-2 text-white font-semibold py-3 px-8 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-md ${
                    selectedOption === 'post' ? 'bg-purple-600 hover:bg-purple-700' :
                    selectedOption === 'reel' ? 'bg-pink-600 hover:bg-pink-700' :
          ...
          <div className="w-full max-w-5xl">
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
              <div className={`grid grid-cols-1 ${selectedOption === 'story' ? 'lg:grid-cols-1' : 'lg:grid-cols-5'} gap-0`}>
                {/* Preview Section */}
                <div className={`${selectedOption === 'story' ? 'lg:col-span-1' : 'lg:col-span-3'} bg-black flex items-center justify-center ${
                  selectedOption === 'story' ? 'min-h-[500px] lg:min-h-[600px]' : 'min-h-[300px] lg:min-h-[500px]'
                } relative`}>
                  {mediaType === 'video' ? (
                    <video src={frontendmedia} controls className="max-h-full w-auto max-w-full" />
                  ) : (
                    <img src={frontendmedia} alt="Preview" className="max-h-full w-auto max-w-full object-contain" />
                  )}
                  <button
                  ...
                  <div className="lg:col-span-2 p-6 lg:p-8 flex flex-col">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-xl font-bold text-gray-800">
                        {selectedOption === 'post' ? 'Add Post Details' : 'Add Reel Details'}
                      </h3>
                    </div>
                    <textarea
                      value={caption}
                    ...
                    <div className={`mt-4 text-xs flex items-center gap-2 ${
                      selectedOption === 'post' ? 'text-purple-600' : 'text-pink-600'
                    }`}>
                      <div className={`w-2 h-2 rounded-full ${
                        selectedOption === 'post' ? 'bg-purple-600' : 'bg-pink-600'
                      }`}></div>
                      <span>Your {selectedOption} will be shared with your followers</span>
                    </div>
                  </div>
                )}
              </div>
