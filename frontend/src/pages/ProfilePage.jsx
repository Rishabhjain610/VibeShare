...
const PostGrid = ({ posts, emptyStateConfig }) => {
  ...
  return posts.map(
    (post) =>
      post &&
      post._id && ( // Ensure post and post._id exist
        <div
          key={post._id}
...
const ProfilePage = () => {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      dispatch(
        setProfileData({
          user: { ...profileData, followers: updatedFollowers },
        })
      );
    } catch (error) {
  ...
  const emptyStateConfig =
    activeTab === "posts"
      ? {
          icon: <Grid size={40} className="text-gray-400 md:w-12 md:h-12" />,
          title: "No Posts Yet",
          message: "When posts are shared, they'll appear here.",
        }
      : {
          icon: (
            <Bookmark size={40} className="text-gray-400 md:w-12 md:h-12" />
          ),
                ...
                <div className="flex gap-2 md:gap-3 w-full md:w-auto">
                  {isOwnProfile ? (
                    ...
                    <>
                      ...
                      <button
                        className="flex-1 md:flex-none bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold px-4 md:px-6 py-2 rounded-lg transition-colors text-sm md:text-base"
                        onClick={() => {
